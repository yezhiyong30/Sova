...
IO_DATA_SIZE = struct.calcsize(IO_DOM_INFO)*NDOMAINS
IO_STATES_SIZE = struct.calcsize(IO_STATES_INFO)*NDOMAINS

# location of mmaped file, hard coded right now
SHM_FILE = "/var/run/xenq-shm"
IO_STATES_FILE="/var/run/sova/io-stats"
...
# the vm's stats monitoring code
def data_stats():
    ncpu = 1         # number of cpu's on this platform
    slen = 0         # size of shared data structure, incuding padding
    ...
    statsf = open(IO_STATES_FILE, "w+")
    memset = []
    memset.append(struct.pack(IO_STATES_INFO, 0, 0, 0.0, 0))
    memset = memset*NDOMAINS
    statsf.write(''.join(memset))
    statsf.flush()
    statsf.close
    ...
    statsf = open(IO_STATES_FILE, "r+")
    stats = mmap.mmap(statsf.fileno(), IO_STATES_SIZE)
...
#               dom_in_use.append(in_use)
            ...
            # get summary over desired interval(10s)
            [h1, l1, f1] = summarize(startat, endat, 10 * 10**9, samples)

            # accumulate all CPU data
            for dom in range(0, NDOMAINS):
                if not dom_in_use[dom]:
                    gotten[dom] = 0
                    wait[dom] = 0
                    blocked[dom] = 0
                    continue
                if domain_id[dom] != IDLE_DOMAIN :
                    gotten[dom] = gotten[dom] + h1[dom][0][0]
                    wait[dom] = wait[dom] + h1[dom][3][0]
                    blocked[dom] = blocked[dom] + h1[dom][2][0]
        ...
        for dom in range(0, NDOMAINS):
            ...
            if domain_id[dom] != IDLE_DOMAIN :
                if gotten[dom] != 0:
                    ratio[dom] = blocked[dom] / gotten[dom]
                    #ratio[dom] = wait[dom]*100 / gotten[dom]
                else :
                    ratio[dom] = blocked[dom]
                    #ratio[dom] = wait[dom]*100
        print ratio
        #print ratio
        
        # write data to file
        for dom in range(0, NDOMAINS):
            len = struct.calcsize(IO_STATES_INFO)
            stats[dom*len:(dom+1)*len] = ''.join(struct.pack(IO_STATES_INFO, dom_in_use[dom], domain_id[dom], ratio[dom]), gotten[dom])
        stats.flush()
