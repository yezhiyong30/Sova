...
#define VM_STATS_FILE "/var/run/sova/vm-stats"
...
static void destroy_domain(uint32_t domid, int force)
{
    int rc;
    FILE *stats_fd = NULL;
    struct dom_stats vm_stats;

    vm_stats.type = 2;
    vm_stats.domid = domid;

    if((stats_fd = fopen(VM_STATS_FILE, "a+")) == NULL)
        perror("cannot open stats file");
    fwrite(&vm_stats, sizeof(struct dom_stats), 1, stats_fd);
    fclose(stats_fd);
